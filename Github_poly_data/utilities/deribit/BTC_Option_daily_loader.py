...
from utilities.deribit.BTC_Option_Chain import BTC_Option_Chain

SCHEMA_NAME = "Crypto_Option"
TABLE_MAIN = "deribit_btc_options"
TABLE_SYN  = "deribit_btc_options_syn"
RUN_TIME_UTC = "00:10"

# ▼▼▼ 关键改动：在表结构中增加 greeks 字段 ▼▼▼
CREATE_SQL = """
...
def pg_batch_insert(df, table_base_name, symbol_name="btc"):
    if df.empty:
        return
    df = df.copy()

    df['run_ts'] = pd.to_datetime(df['utc_ts'])
    table_name = f"{table_base_name}_{symbol_name}_{df['run_ts'].iloc[0].strftime('%Y%m%d')}"

    conn = get_connection()
    try:
        with conn.cursor() as cur:
            cur.execute(sql.SQL("CREATE SCHEMA IF NOT EXISTS {}").format(sql.Identifier(SCHEMA_NAME)))
            cur.execute(sql.SQL(CREATE_SQL).format(schema=sql.Identifier(SCHEMA_NAME), table=sql.Identifier(table_name)))

            # ▼▼▼ 关键改动：在待插入列中增加 greeks 字段 ▼▼▼
            columns_to_insert = [
                "run_ts", "spot_px", "symbol", "type", "strike", "expiry",
                "iv", "underlying", "contractSize",
                "bid_coin", "ask_coin", "bid_usd", "ask_usd",
...
def daily_job():
    logging.info("⏳  Fetching Deribit BTC option chain (with spot price, volume, and greeks)...")
    try:
        df_main, df_syn, df_skip = BTC_Option_Chain()
        pg_batch_insert(df_main, TABLE_MAIN, symbol_name="btc")
        pg_batch_insert(df_syn, TABLE_SYN,  symbol_name="btc")
