#!/usr/bin/env python3
# option_poly/deribit_poly.py — 每个 slug 独立表
...
def exp_from_token(tok:str):
    ...
    tok = tok.zfill(6)
    return datetime(2000+int(tok[:2]), int(tok[2:4]), int(tok[4:]),
                    tzinfo=timezone.utc)

def deribit_chain():
    flat = EXCHANGE.fetch_option_chain("BTC")
    return [{"symbol":s.split(":")[-1], **d, **d["info"]}
            for s,d in flat.items()]

def match_deribit(strike, pm_exp, opt_type):
    letter = "P" if opt_type=="put" else "C"
    pool=[r for r in deribit_chain()
          if r["symbol"].split("-")[3].upper()==letter
          and abs(float(r["symbol"].split("-")[2])-strike)<=1]
    same=[r for r in pool if exp_from_token(r["symbol"].split("-")[1]).date()==pm_exp.date()]
    if not same:
...
COLS = ("ts_utc,slug,pm_expiry,pm_yes_bid,pm_yes_ask,pm_no_bid,pm_no_ask,"
        ...
        with conn,conn.cursor() as cur:
            cur.execute(sql.SQL(
                ...
                .format(schema=sql.Identifier(SCHEMA),
                        table =sql.Identifier(table)), row)
...
def main(slug:str):
    ...
    pm_expiry=datetime.strptime(ev["endDate"], "%Y-%m-%dT%H:%M:%SZ")\
    ...
    while True:
        ...
        try:
            ...
            iv=(tkr.get("mark_iv") or tkr.get("impliedVolatility")
                or tkr["info"].get("markIv")); iv=float(iv) if iv else None
        except Exception as e:
            logging.warning("Deribit err %s",e); time.sleep(5); continue

        ts=datetime.now(timezone.utc)
        insert_row(table,(ts,slug,pm_expiry,pyb,pya,pnb,pna,
...
if __name__=="__main__":
    if len(sys.argv)<2:
        print("Usage: python option_poly/deribit_poly.py <event-slug>")
        sys.exit(1)
    main(sys.argv[1])
